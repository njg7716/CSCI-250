...
sim_loop:
        slt     $s3, $a0, $a1           #s3 is 1 if den is less than s0 num
        bne     $s3, $zero, sim_else    #if den > num go to else
        sub     $a0, $a0, $a1
        addi    $s1, $s1, 1
        j       sim_loop

sim_else:
        move    $s4, $a0        #save the num before you get GCD
        move    $s5, $a1        #save the den before you get GCD
        move    $s6, $s1        #save the int before you get GCD
        beq     $s4, $zero, sim_special #check if the num is 0
        jal     find_gcd 
        div     $s4, $s4, $v0   #divide the num and den by GCD
        div     $s5, $s5, $v0
        move    $v0, $s4        #move the final values to the correct register
        move    $v1, $s5
        move    $t9, $s6
        j       simplify_done

sim_special:                    #if the num is 0 divide the den by itself to get one
        move    $v0, $s4        #move the final values to the correct register
        move    $v1, $s5
        move    $t9, $s6
        div     $v1, $v1, $v1
...
gcd_loop:
        move    $v0, $a0        #move value to this register so that it is ready when it retruns
        beq     $a0, $a1, find_gcd_done #check if they are equal
        sub     $t9, $a0, $a1
        slt     $t1, $t9, $zero
        bne     $t1, $zero, gcd_absval  #make sure there are no negatives
        slt     $t2, $a1, $a0
        beq     $t2, $zero, gcd_else    #check which is bigger so you can make that the new D
        move    $a0, $t9
        j       gcd_loop
...
gcd_absval:
        sub     $t9, $a1, $a0
        slt     $t2, $a1, $a0
        beq     $t2, $zero, gcd_else    #check which is larger
        move    $a0, $t9 
        j       gcd_loop
