...
block_make:
	...
	la	$t1, block_vtb1	# get block's vtable pointer
...
block_area:
	...
	lw	$t1, 8($a0)	                # get fig's height
        mul     $v0, $t0, $t1                   # height multiplied by width
...
block_perimeter:
	...
	lw	$t1, 8($a0)	# get fig's height
        ...
        add     $t0, $t0, $t1   # add the height and width
        li      $t1, 2          # multiply by 2
        mul     $v0, $t0, $t1
...
# the address of the figure object, and shouldn't be changed.
        lw      $t0, 0($s0)     #gets the address to table
        lw      $t0, 0($t0)     #gets address to area func
        move    $a0, $s0
        jalr    $t0
        move    $a0, $v0
        li      $v0, PRINT_INT
...
# contains the address of the figure object, and shouldn't be changed.
        ...
        lw      $t0, 0($s0)     #gets the address to table
        lw      $t0, 4($t0)     #gets address to perimeter func
        move    $a0, $s0
        jalr    $t0
        move    $a0, $v0
        li      $v0, PRINT_INT
